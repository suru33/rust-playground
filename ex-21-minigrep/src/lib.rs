...
impl Config {
    ...
    pub fn new(args: &[String]) -> Result<Config, &str> {
        if args.len() == 3 {
            ...
            Ok(Config { query, file_name, case_sensitive: true })
        } else if args.len() == 4 {
            ...
            if flag.eq("-i") {
                Ok(Config { query, file_name, case_sensitive: false })
            } else {
                Err(Config::ERR_MSG)
            }
        } else {
...
fn search(query: &str, contents: &str) -> Vec<QueryResult> {
    ...
    for (ix, line) in contents.lines().enumerate() {
        if line.contains(query) {
            results.push(QueryResult { line: (ix + 1) as u32, text: line.to_string() });
        }
    }
    results
}
...
Duct tape.";
        ...
        let expected = vec![QueryResult { line: 2, text: "safe, fast, productive.".to_string() }];
...
Duct tape.";
        ...
        let expected = vec![QueryResult { line: 1, text: "Rust:".to_string() }];
